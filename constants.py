# constants.py
# contains: constants used in all calculations, plus benchmark values

# Common to all runs:
pi = 3.1415926535897932384626433
e = 2.71828182845904523536028747135266249775724709369995
R = 8.3144598  # Ideal gas
...
STO = {
    ...
    'Mp': Me,
    'Mc': Me-4.06e24,
    'Rc': Re - 2.8e6, #3485.0e3,
    #'Qe': (0.75 * 72.0e-3 * (4*pi*Re*Re))/(np.exp( 4.06e24, #(4.86398 * (0.75*36.5e12))/(4.06e24),
    'Vm': 4.06e24/3400}

STO['Qe'] = ((STO['urey'] * 60.0e-3 * (4*pi*Re*Re))/(e**(-1 * STO['decay'] * 4.5)))/(Me-STO['Mc'])  #4.06e24, #(4.86398 * (0.75*36.5e12))/(4.06e24),
print(STO['Mp']-STO['Mc'] - STO['pm']*STO['Vm'])